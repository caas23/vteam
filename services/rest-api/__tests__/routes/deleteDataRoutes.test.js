...
describe('deleteDataRoutes.js', () => {
    ...
    it('GET / - should return available delete routes', async () => {
        const response = await request(server).get('/v1/delete');
    ...
    it('DELETE /v1/:city/:type/:id - should delete a parking from a city', async () => {
        ...
        const response = await request(app).delete(`/v1/delete/${mockCity}/${mockType}/${parking_id}`)
        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty('acknowledged', true);
        expect(response.body).toHaveProperty('deletedCount', 1);
    });

    it('DELETE /v1/:city/:type/:id - should return 500 if type is invalid', async () => {
        const mockCity = 'Lund';
        const invalidType = 'invalidType';
        const parking_id = 'P016';
    
        const response = await request(app).delete(`/v1/delete/${mockCity}/${invalidType}/${parking_id}`)
        expect(response.status).toBe(500);
        expect(response.body).toHaveProperty('error', 'An error occurred while deleting the item.');
    });

    it('DELETE /v1/bike/:bike_id - should delete a bike', async () => {
        ...
        const response = await request(app).delete(`/v1/delete/bike/${bike_id}`)
        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty('acknowledged', true);
        expect(response.body).toHaveProperty('deletedCount', 1);
    });
    
    it('DELETE /v1/user/:user_id - should delete a user', async () => {
        ...
        const response = await request(app).delete(`/v1/delete/user/${user_id}`)
        expect(response.status).toBe(200);
        expect(response.body).toHaveProperty('acknowledged', true);
        expect(response.body).toHaveProperty('deletedCount', 1);
    }); 
